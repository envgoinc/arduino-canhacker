...
CAN_ERROR MCP2515_CAN::getError(){
  return _err;
}
...
CAN_ERROR MCP2515_CAN::connectCan(){
...
CAN_ERROR MCP2515_CAN::checkErrorCounter() {
  char out[15];
  CAN_ERROR error;
...
CAN_ERROR MCP2515_CAN::createErrorStatus(const char error, char *buffer, const int length) {
    uint16_t i = 0;
    if (length < 5) {
...
CAN_ERROR MCP2515_CAN::createBusState(const char state, char *buffer, const int length) {
    uint16_t i = 0;
